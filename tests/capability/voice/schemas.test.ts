...
describe('VoiceToolNameSchema', () => {
  ...
  it('enum covers all 13 D-16 tools (12 original + confirm_recipient_email from Bug A structural fix)', () => {
...
describe('VoiceCallOutcomeSchema (16 fields per ADR §14.2)', () => {
  ...
  it('accepts unknown recipient_sentiment via VoiceRecipientSentimentLenientSchema (post-Auditor B-02 widening, commit 061aeef)', () => {
    // Sentiment field uses the lenient schema which accepts unknown string
    // values (including empty string) to preserve ElevenLabs analysis data
    // even when the provider emits an unrecognized sentiment. Normalization
    // happens downstream via normalizeSentiment() helper.
    // Previously (pre-widening) this test asserted .success === false.
    expect(
